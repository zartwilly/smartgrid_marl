...
def find_out_min_max_bg(arr_pl_M_T_K_vars, arr_bg_i_nb_repeat_k, 
    ...
    #         = np.nanmax(
    #             arr_pl_M_T_K_vars[
    #                 indices_playing_players_pl_is,t,
    #                 0:k+1, 
...
def utility_function_version1(arr_pl_M_T_K_vars, 
    ...
    print("bg_i_0_k+1 = {},".format(
        arr_pl_M_T_K_vars[:,t,0:k+1,fct_aux.INDEX_ATTRS["bg_i"]]))
    ...
    print("bg_min_i_t_0_to_k= {}, bg_max_i_t_0_to_k= {}".format(bg_min_i_t_0_to_k, bg_max_i_t_0_to_k))
    ...
    # comp_nan_bg_min_max_not_players = True
    # if (np.argwhere(np.isnan(bg_max_i_t_0_to_k)).reshape(-1) \
    #     == np.argwhere(np.isnan(bg_min_i_t_0_to_k)).reshape(-1)).all() \
    #         and (np.argwhere(np.isnan(bg_max_i_t_0_to_k)).reshape(-1) \
    #             == indices_non_playing_players).all():
    #     comp_nan_bg_min_max_not_players = False
    #if (np.isnan(bg_max_i_t_0_to_k)).all() 
    ...
    # print("u_i_t_k = {}".format(arr_pl_M_T_K_vars[indices_playing_players_pl_is,t,k,fct_aux.INDEX_ATTRS["u_i"]]))
    # print("p_i_t_k_new={}".format(arr_pl_M_T_K_vars[indices_playing_players_pl_is,t,k,fct_aux.INDEX_ATTRS["prob_mode_state_i"]]))
    ...
    #p_i_t_k_new = p_i_t_k + learning_rate * u_i_t_k * (1 - p_i_t_k)
    print("NaN? u_i_t_k={}".format(np.argwhere(np.isnan( u_i_t_k ).reshape(-1))))
    ...
    for num_pl_i in range(0,arr_pl_M_T_K_vars.shape[0]):
        if np.isnan(u_i_t_k[num_pl_i]):
            ...
            print('1 INSIDE pl_i={}, u_i_t_k={}, p_i_t_k_new={}'.format(num_pl_i,u_i_t_k[num_pl_i], p_i_t_k_new[num_pl_i]))
        else:
            p_i_t_k_new[num_pl_i] = p_i_t_k[num_pl_i] \
                                    + learning_rate \
                                        * u_i_t_k[num_pl_i] \
                                            * (1 - p_i_t_k[num_pl_i])
            print('2 INSIDE pl_i={}, u_i_t_k={}, p_i_t_k_new={}'.format(num_pl_i,u_i_t_k[num_pl_i], p_i_t_k_new[num_pl_i]))
...
def utility_function_version2(arr_pl_M_T_K_vars, arr_bg_i_nb_repeat_k,
    ...
    print("bg_i_s={}, bens_t_k={}, csts_t_k={}, Ci={}, Pi={}".format(
        arr_pl_M_T_K_vars[indices_playing_players_pl_is,t,k,
                          fct_aux.INDEX_ATTRS["bg_i"]], 
        bens_t_k, csts_t_k,
        arr_pl_M_T_K_vars[indices_playing_players_pl_is,t,k,
                          fct_aux.INDEX_ATTRS["Ci"]], 
    ...
    if comp_min_max_bg.any() == True:
        ...
        for num_pl_i in indices_non_playing_players:
            state_i = arr_pl_M_T_K_vars[num_pl_i,t,k,fct_aux.INDEX_ATTRS["state_i"]]
            mode_i = arr_pl_M_T_K_vars[num_pl_i,t,k,fct_aux.INDEX_ATTRS["mode_i"]]
            print("#### 12 num_pl_i={}, state={}, mode={}".format(num_pl_i, state_i, mode_i))    
        print("#### 1")
        return arr_pl_M_T_K_vars, arr_bg_i_nb_repeat_k, \
                bool_bg_i_min_eq_max, list(indices_non_playing_players),\
                bg_min_i_t_0_to_k, bg_max_i_t_0_to_k
    ...
    arr_pl_M_T_K_vars[
        ...
        t, k,
        fct_aux.INDEX_ATTRS["u_i"]] = u_i_t_k
    print("u_i_t_k = {}".format(
        arr_pl_M_T_K_vars[indices_playing_players_pl_is,t,k,
                          fct_aux.INDEX_ATTRS["u_i"]]))
    print("p_i_t_k_new={}".format(
        arr_pl_M_T_K_vars[indices_playing_players_pl_is,t,k,
                          fct_aux.INDEX_ATTRS["prob_mode_state_i"]]))
...
def balanced_player_game_t(arr_pl_M_T_K_vars, t, k, 
    ...
    for num_pl_i in indices_playing_players_pl_is:
        ...
        # ## update Si at t==0
        # if t==0 and nb_repeat_k<1 and state_i == fct_aux.STATES[0]:
        #     if Ci-Pi-1 == 0:
        #         Pi -= 1
        #     high = min(Si_max, Ci-Pi-1)
        #     Si = np.random.uniform(low=1, high=high)
        # elif t==0 and nb_repeat_k<1 and state_i == fct_aux.STATES[1]:
        #     low = min(Ci-Pi, Si_max)
        #     high = min(2*(Ci-Pi), Si_max)
        #     Si = np.random.uniform(low=low, high=high)
        # elif t==0 and nb_repeat_k<1 and state_i == fct_aux.STATES[2]:
        #     low = 1
        #     high = Si_max
        #     Si = np.random.uniform(low=low, high=high)
        # pl_i.set_Si(Si)
        ...
        print("Si={}, p_i_t_k={}".format(Si, p_i_t_k))
        pl_i.select_mode_i(p_i=p_i_t_k)
        
        pl_i.update_prod_cons_r_i()
        print("Si={}, Si_pli={}".format(Si, pl_i.get_Si()))
    ...
    bens_t_k, csts_t_k = fct_aux.compute_utility_players(
                            ...
                            b0_t_k, 
                            c0_t_k)
    print("b0_t_k={}, c0_t_k={}, prod_t_k={}, cons_t_k={}, r_t_k={}, gamma_is={}".format(
        b0_t_k, c0_t_k, arr_pl_M_T_K_vars[:, t, k, fct_aux.INDEX_ATTRS["prod_i"]], 
        arr_pl_M_T_K_vars[:, t, k, fct_aux.INDEX_ATTRS["cons_i"]],
        arr_pl_M_T_K_vars[:, t, k, fct_aux.INDEX_ATTRS["r_i"]],
...
def lri_balanced_player_game_old(arr_pl_M_T,
                             pi_hp_plus=0.10, 
                             pi_hp_minus=0.15,
                             m_players=3, 
                             num_periods=4, 
        ...
        while (k < k_steps):
            ...
            if nb_repeat_k == fct_aux.NB_REPEAT_K_MAX:
                ...
                for var in ["bg_i", "mode_i", "prod_i", "cons_i",
                    ...
                    #                             t,k,fct_aux.INDEX_ATTRS[var]].shape, arr_pl_M_T_K_vars_modif[indices_non_playing_players,t,k:k_steps,fct_aux.INDEX_ATTRS[var]].shape))
                    print("SHAPE NON_PLAYING: t={},k={},k_steps={} var_modifs_non_players={}, k:k_steps={}".format(
                        t,k,k_steps,
                        arr_pl_M_T_K_vars_modif[indices_non_playing_players,
                    ...
                    arr_pl_M_T_K_vars[
                        ...
                        fct_aux.INDEX_ATTRS[var]] \
                                = arr_pl_M_T_K_vars_modif[
                                    ...
                                    t,k,
                                    fct_aux.INDEX_ATTRS[var]].reshape(-1,1)
                print("AFTER UPDATE k+1->k-step; var mode_i={}, \n var_modif mode_i={}".format(
                    arr_pl_M_T_K_vars[
                        indices_non_playing_players,
                        t,k+1:k_steps,
                ...
                arr_bg_i_nb_repeat_k.fill(np.nan)
                ## test indices_non_playing_players --> fin
