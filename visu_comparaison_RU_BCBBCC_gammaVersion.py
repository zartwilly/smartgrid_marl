# -*- coding: utf-8 -*-
"""
Created on Mon Mar 29 16:11:10 2021
...
def get_tuple_paths_of_arrays(name_dirs=["tests"], 
                algos_4_no_learning=["DETERMINIST","RD-DETERMINIST",
                                     "BEST-BRUTE-FORCE","BAD-BRUTE-FORCE", 
                                       "MIDDLE-BRUTE-FORCE"], 
                algos_4_showing=["DETERMINIST", "LRI1", "LRI2",
    ...
    for dp in lis_old:
        if len(dp.split(os.sep)) > 2:
            tuple_paths.append( tuple(dp.split(os.sep)) )
            if dp.split(os.sep)[3] in ["LRI1", "LRI2"]:
...
def get_array_turn_df_for_t(tuple_paths, t=1, k_steps_args=250, 
    ...
    """
    TODO
    ajouter la version comme une colonne dans le tableau
    """
    df_arr_M_T_Ks = []
...
def plot_gamma_version(df_ra_pr, rate, price):
...
def plot_comparaison_gamma_version(df_B_C_BB_CC_RU_M):
    ...
    for rate, price in it.product(rates, prices):
        ...
        pxs_pr_ra = plot_gamma_version(df_ra_pr, rate, price)
...
if __name__ == "__main__":
    ...
    # excellent fonctionne correctement sur le rep TESTS_CP
    ...
    df_b0_c0_pisg_pi0_T_K, df_B_C_BB_CC_RU_M \
        = get_array_turn_df_for_t(tuple_paths, t=1, k_steps_args=250)
    ...
    rows_RU_C_B_CC_BB = plot_comparaison_gamma_version(df_B_C_BB_CC_RU_M)
    tab_compGammaVersion = Panel(child=rows_RU_C_B_CC_BB, title="comparison Gamma_version")
    tabs = Tabs(tabs= [ tab_compGammaVersion])
    output_file( os.path.join(name_dir, "comparaison_RU_BCBBCC_gammaVersion.html")  )
    #save(tabs)
    show(tabs)
