...
def bf_balanced_player_game_USE_DICT_MODE_PROFIL_SANS_DICO_COMBINAISON(
                            ...
                            manual_debug=False, 
                            criteria_bf="Perf_t", dbg=False):
    # TODO: erreur sur la determination du Perf_t moyen
    """
    brute force algorithm for balanced players' game.
    determine the best solution by enumerating all players' profils.
    The comparison critera is the Pref_t value with
    ...
    for t in range(0, t_periods):
        ...
        dico_modes_profs_by_players = dict()
        Perf_ts = list()
        mode_profile_BEST, mode_profile_MID, mode_profile_BAD = None, None, None
        dico_best_mode_profs, dico_bad_mode_profs, dico_mid_mode_profs \
                = dict(), dict(), dict()
        list_dico_best_mode_profs, list_dico_bad_mode_profs, list_dico_mid_mode_profs \
            = list(), list(), list()
        best_mode_profiles, bad_mode_profiles, mid_mode_profiles \
            = list(), list(), list()
        Vi_max = -np.inf
        Vi_min = np.inf
        Vi_mid = np.nan
        ...
        for mode_profile in mode_profiles:
            ...
            if Vi_max <= Perf_t:
                Vi_max = Perf_t
                dico_best_mode_profs \
                    = get_mode_prof_by_players(
                        arr_pl_M_T_vars_mode_prof, t, Vi_max,
                        bens_t, csts_t, bens_csts_t, 
                        b0_t, c0_t, In_sg, Out_sg, 
                        pi_sg_plus_t, pi_sg_minus_t, 
                        pi_0_plus_t, pi_0_minus_t, m_players)
                list_dico_best_mode_profs.append(
                    ("BF"+str(cpt_xxx), dico_best_mode_profs)
                )
                mode_profile_BEST = mode_profile
                best_mode_profiles.append(mode_profile)
                    
            elif Vi_min >= Perf_t:
                Vi_min = Perf_t
                dico_bad_mode_profs \
                    = get_mode_prof_by_players(
                        arr_pl_M_T_vars_mode_prof, t, Vi_min,
                        bens_t, csts_t, bens_csts_t,
                        b0_t, c0_t, In_sg, Out_sg, 
                        pi_sg_plus_t, pi_sg_minus_t, 
                        pi_0_plus_t, pi_0_minus_t, m_players)
                list_dico_bad_mode_profs.append(
                    ("BF"+str(cpt_xxx), dico_bad_mode_profs)
                )
                mode_profile_BAD = mode_profile
                bad_mode_profiles.append(mode_profile)
                    
            Perf_ts.append(Perf_t)
            if np.isnan(Vi_mid):
                Vi_mid = np.mean(Perf_ts)
                dico_mid_mode_profs \
                    = get_mode_prof_by_players(
                        arr_pl_M_T_vars_mode_prof, t, Perf_t,
                        bens_t, csts_t, bens_csts_t,
                        b0_t, c0_t, In_sg, Out_sg, 
                        pi_sg_plus_t, pi_sg_minus_t, 
                        pi_0_plus_t, pi_0_minus_t, m_players)
                list_dico_mid_mode_profs.append(
                    ("BF"+str(cpt_xxx), dico_mid_mode_profs)
                )
                mode_profile_MID = mode_profile
            else:
                Vi_mid = np.mean(Perf_ts)
                if Perf_t >= Vi_mid*(1 - 0.01) and Perf_t <= Vi_mid*(1 + 0.01):
                    dico_mid_mode_profs \
                    = get_mode_prof_by_players(
                        arr_pl_M_T_vars_mode_prof, t, Perf_t,
                        bens_t, csts_t, bens_csts_t,
                    ...
                    mode_profile_MID = mode_profile
                    mid_mode_profiles.append(mode_profile)
        ...
        for algo_name in fct_aux.ALGO_NAMES_BF:
            ...
            mode_profiles_algo = None
            list_dico_ALGO_mode_profs = None
            if algo_name == fct_aux.ALGO_NAMES_BF[0]:              # BEST-BRUTE-FORCE
                arr_pl_M_T_vars_modif_algo = arr_pl_M_T_vars_modif_BESTBF.copy()
                pi_sg_plus_T_algo = pi_sg_plus_T_BESTBF.copy()
                pi_sg_minus_T_algo = pi_sg_minus_T_BESTBF.copy()
                ...
                pi_0_minus_T_algo = pi_0_minus_T_BESTBF.copy()
                dico_stats_res_algo = dico_stats_res_BESTBF.copy()
                selected_mode_profile = mode_profile_BEST
                mode_profiles_algo = best_mode_profiles
                list_dico_ALGO_mode_profs = list_dico_best_mode_profs
                ...
                pi_0_minus_T_algo = pi_0_minus_T_BADBF.copy()
                dico_stats_res_algo = dico_stats_res_BADBF.copy()
                selected_mode_profile = mode_profile_BAD
                mode_profiles_algo = bad_mode_profiles
                list_dico_ALGO_mode_profs = list_dico_bad_mode_profs
                ...
                pi_0_minus_T_algo = pi_0_minus_T_MIDBF.copy()
                dico_stats_res_algo = dico_stats_res_MIDBF.copy()
                selected_mode_profile = mode_profile_MID
                mode_profiles_algo = mid_mode_profiles
                list_dico_ALGO_mode_profs = list_dico_mid_mode_profs
            
            
            # ____      game beginning for one algo for t ---> debut      _____
            Perf_t_algo = None
            if algo_name.split("-")[0] == "BEST":
                Perf_t_algo = Vi_max 
            elif algo_name.split("-")[0] == "MIDDLE":
                Perf_t_algo = Vi_mid
            elif algo_name.split("-")[0] == "BAD":
                Perf_t_algo = Vi_min 
            print("cpt_xxx={}, Perf_t={}, {}_mode_profile={}".format(
                    cpt_xxx, Perf_t_algo, algo_name.split("-")[0], 
                    selected_mode_profile))
            ...
            dico_gamme_t \
                = balanced_player_game_4_mode_profil_prices_SG(
                    arr_pl_M_T_vars_modif_algo.copy(),
                    selected_mode_profile, t,
                    pi_hp_plus, pi_hp_minus,
                    pi_0_plus_t, pi_0_minus_t,
                    manual_debug, dbg)
            dico_stats_res[t] = dico_gamme_t
            suff = algo_name.split("-")[0]
            dico_stats_res_algo[t] = {
                    "gamma_i": dico_gamme_t,
                     suff+"_mode_profiles": mode_profiles_algo,
                     "nb_"+suff+"_mode_profiles": len(mode_profiles_algo),
                     suff+"_mode_profile": selected_mode_profile,
                     suff+"_Perf_t": Perf_t_algo,
                     "list_dico_best_mode_profs": 
                         list_dico_ALGO_mode_profs,
                     suff+"_b0_t": b0_t,
                     suff+"_c0_t": c0_t
                                }
        
            # verification of best key quality 
            diff = np.abs(Perf_t_algo -( np.sum(bens_t-csts_t) ))
            print("best_key==bens_t-csts_t --> OK (diff={}) ".format(diff)) \
                if diff < 0.1 \
                else print("best_key==bens_t-csts_t --> NOK (diff={}) \n".format(diff))
            In_sg, Out_sg = fct_aux.compute_prod_cons_SG(
                                arr_pl_M_T_vars_mode_prof_best_algo, t)
            print("b0_t={}, c0_t={}, Out_sg={},In_sg={}".format(b0_t, c0_t, Out_sg, In_sg))
...
def test_brute_force_game_USE_DICT_MODE_PROFIL_SANS_DICO_COMBINAISON(criteria):
    fct_aux.N_DECIMALS = 8
