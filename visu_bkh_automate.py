...
# _____________________________________________________________________________ 
def plot_max_proba_mode_onestate_BON_A_REVENIR_QUAND_NEW_IMPLEMENTATION_NON_OK(df_ra_pri_st, 
...
# _____________________________________________________________________________
#
#                   debug plot_max_proba_mode_onestate  ---> debut
# _____________________________________________________________________________
def debug_plot_max_proba_mode_onestate(df_arr_M_T_Ks, t=1):
    rates = df_arr_M_T_Ks["rate"].unique(); rates = rates[rates!=0]
    prices = df_arr_M_T_Ks["prices"].unique()
    states = df_arr_M_T_Ks["state_i"].unique()
    algos = df_arr_M_T_Ks["algo"].unique()
    
    rate = rates[0] if rates[0] != 0 else rates[1]
    price = prices[0];
    state_i = states[0]
    algo = algos[0]
    
    mask_ra_pri_st_al = ((df_arr_M_T_Ks.rate == rate) 
                                 | (df_arr_M_T_Ks.rate == 0)) \
                    & (df_arr_M_T_Ks.prices == price) \
                    & (df_arr_M_T_Ks.t == t) \
                    & (df_arr_M_T_Ks.state_i == state_i) \
                    & (df_arr_M_T_Ks.algo == algo)     
    df_ra_pri_st_al = df_arr_M_T_Ks[mask_ra_pri_st_al].copy()
    
    df_al = pd.DataFrame(); 
    # df_al["S1"] = df_ra_pri_st_al["p_i_j_k"].apply(
    #                 pd.to_numeric,downcast='float',
    #                 errors='coerce'); 
    df_al["S1"] = df_ra_pri_st_al["u_i"].apply(
                    pd.to_numeric,downcast='float',
                    errors='coerce');
    df_al.loc[:,"S2"] = 1 - df_al["S1"]; 
    df_al["k"] = df_ra_pri_st_al["k"];
    
    df_al_k = df_al.groupby("k")[["S1","S2"]].agg(
                                    {"S1": [np.mean, np.min, np.max, np.std], 
                                     "S2": [np.mean, np.min, np.max, np.std]})
    tuple_cols = list(df_al_k.columns)
    dico_new_cols = dict()
    for tu_col in tuple_cols:
        dico_new_cols[tu_col] = tu_col[0]+"_"+tu_col[1]
        
    df_al_k.columns = df_al_k.columns.to_flat_index()
    df_al_k.rename(columns=dico_new_cols, inplace=True)
    
    df_al_k['S1_lower'] = df_al_k.S1_mean - df_al_k.S1_std
    df_al_k['S1_upper'] = df_al_k.S1_mean + df_al_k.S1_std
    
    df_al_k['S2_lower'] = df_al_k.S2_mean - df_al_k.S2_std
    df_al_k['S2_upper'] = df_al_k.S2_mean + df_al_k.S2_std
    
    source = ColumnDataSource(df_al_k.reset_index())

    TOOLS = "pan,wheel_zoom,box_zoom,reset,save"
    p = figure(tools=TOOLS)
    
    p.scatter(x='k', y='S1_mean', line_color=None, 
              fill_alpha=0.3, size=5, source=source)
    
    band = Band(base='k', lower='S1_lower', upper='S1_upper', 
                source=source, level='underlay',
                fill_alpha=1.0, line_width=1, line_color='black')
    p.add_layout(band)
...
# _____________________________________________________________________________
#
#                   debug plot_max_proba_mode_onestate  ---> fin
# _____________________________________________________________________________
...
if __name__ == "__main__":
    ...
    #debug_plot_max_proba_mode_onestate(df_arr_M_T_Ks)
